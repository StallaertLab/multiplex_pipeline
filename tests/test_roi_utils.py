...
def test_pre_select_intensity_calculation(mock_image):
    ...
    filtered_high = roi_utils.pre_select_objects(mask, mock_image, 1, 1000, 0, 0, min_int=101)
...
def test_remove_overlapping_objects_priority():
    ...
    objects = [
        ...
        {"bbox": [100, 100, 10, 10], "predicted_iou": 0.7, "id": "distinct"} # No overlap
    ]
...
def test_prepare_poly_df_for_saving_integration():
    ...
    df = roi_utils.prepare_poly_df_for_saving(poly_data, poly_types, req_level, org_shape)
