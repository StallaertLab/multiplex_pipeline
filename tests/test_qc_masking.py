...
def test_check_belonging_spatial_index():
    ...
    assert result_mask[0] == False # Inside polygon
    assert result_mask[1] == True  # Outside polygon
...
def test_check_belonging_empty_pairs():
    ...
    assert mask[0] == True # Default is ones (True)
...
def test_build_qc_mask_broadcasting(mock_sdata):
    ...
    assert final_mask[0, 0] == False # MarkerA_mean
    assert final_mask[0, 1] == False # MarkerA_max
    
    # Cols 2 & 3 are MarkerB. No QC shape exists for MarkerB. Should be True (kept).
    assert final_mask[0, 2] == True
    assert final_mask[0, 3] == True
    
    # --- Check Cell 1 (Outside the polygon at 50,50) ---
    # All should be True
    assert final_mask[1, 0] == True
    assert final_mask[1, 2] == True
