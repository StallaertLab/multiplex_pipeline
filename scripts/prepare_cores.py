...
from multiplex_pipeline.core_preparation.file_io import (
    LocalFileStrategy, GlobusFileStrategy
)
...
def configure_logging(output_dir):
    ...
    log_path = os.path.join(output_dir, f"pipeline_{Path(output_dir).stem}_{timestamp}.log")

    logger.add(log_path, level="INFO", backtrace=True, diagnose=True)
    logger.info(f"Logging initialized: {log_path}")

def parse_args():
    parser = argparse.ArgumentParser(description="Prepare cores from OME-TIFFs using metadata and optional Globus transfers.")
    ...
    parser.add_argument("--transfer-cache-dir", help="Optional subfolder under temp-dir for downloaded files")
    parser.add_argument("--transfer-cleanup-enabled", default=True, help="Enable cleanup of transfer cache after processing")
    parser.add_argument("--core-cleanup-enabled", default=True, help="Enable cleanup of temporary core files after assembling SpatialData object")
...
def main():
    ...
    gc = GlobusConfig.from_config_files(Path(cfg["globus_config"])) if cfg.get("globus_config") else None
    ...
    if gc and tc:
        transfer_cache_dir = cfg.get("transfer_cache_dir") or str(Path(cfg["temp_dir"]) / "input_cache")
        transfer_map = build_transfer_map(image_map, transfer_cache_dir)
        strategy = GlobusFileStrategy(tc, transfer_map, gc, cleanup_enabled=cfg['transfer_cleanup_enabled'])
        image_paths = {ch: str(Path(transfer_cache_dir) / Path(remote).name) for ch, (remote, _) in transfer_map.items()}
    else:
