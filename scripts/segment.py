...
from multiplex_pipeline.segmentation.segmenters import DummySegmenter, CellposeSegmenter
...
def main():
    parser = argparse.ArgumentParser(description="Run segmentation on a SpatialData Zarr object.")
    parser.add_argument("--sdata-path", required=True, help="Path to SpatialData .zarr folder")
    parser.add_argument("--config", required=True, help="Path to segmentation YAML config file")
    ...
    if not method or not channels:
        raise ValueError("Config must contain 'method' and 'channels' fields under 'segmentation'.")

    segmenter = build_segmenter(method, config)
    controller = SegmentationController(segmenter)

    print(f"Segmenting {args.sdata_path} using {method} on channels: {channels}")
    controller.segment_spatial_data(Path(args.sdata_path), channels, mask_name=output_name)
