...
class InstansegSegmenter(BaseOp):
    ...
    def validate_config(self, cfg: Mapping[str, Any]) -> None:
        ...
        if not isinstance(cfg, dict) or "model" not in cfg:
            message = f'Instanseg requires specified model, instead got {self.cfg}'
            logger.error(message)
            raise ValueError(message)
        
        # checks if other parameters are instanseg specific
        # logs warning if not
        defined_parameters = ['pixel_size', 'resolve_cell_and_nucleus','cleanup_fragments']
        for param in self.cfg.keys():
        ...
        if 'resolve_cell_and_nucleus' in self.cfg and self.cfg['resolve_cell_and_nucleus']:
    ...
    def prepare_input(self,in_image):
        ...
        if isinstance(in_image, tuple):
            in_image = np.stack(input, axis=-1)  # (H, W, C)
            ...
            # 3D but has to transposed
                ...
                in_image = np.moveaxis(input, 0, -1)
...
class Cellpose4Segmenter(BaseOp):
    ...
    def validate_config(self, cfg: Mapping[str, Any]) -> None:
        ...
        if isinstance(cfg, dict):
            defined_parameters = ['diameter','flow_threshold', 'cellprob_threshold','niter']
    ...
    def prepare_input(self,in_image):
        ...
        if isinstance(in_image, tuple):
            ...
            if len(in_image) > 2:
                logger.warning('Cellpose will use only the first two provided channels.')
