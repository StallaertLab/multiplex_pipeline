...
class BaseSegmenter:
    def run(self, image: np.ndarray) -> np.ndarray:
        raise NotImplementedError(
            "Segmenters must implement the run() method."
        )
...
class CellposeSegmenter(BaseSegmenter):
    ...
    def run(
        self, image: Union[np.ndarray, Sequence[np.ndarray]], **kwargs
    ) -> np.ndarray:
        """
        ...
        if isinstance(image, np.ndarray):
            if image.ndim == 2:
                ...
                channels = kwargs.get("channels") or self.model_kwargs.get(
                    "channels"
                )
                if channels is None:
                    raise ValueError(
                        "The 'channels' keyword argument must be specified for multi-channel Cellpose segmentation."
                    )
                mask, *_ = self.model.eval(
                    image, channels=channels, **self.model_kwargs
                )
                return mask
            else:
                raise ValueError(
                    "If passing a 3D array, shape must be (3, H, W) for multi-channel Cellpose."
                )
        else:
            raise ValueError(
